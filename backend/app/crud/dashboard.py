...
def get_dashboard_stats(db: Session) -> Dict[str, Any]:
    """
    Computes global statistics, ward occupancy statistics,
    daily admission/discharge trends, and lists recent admissions.
    """
    ...
    for ward in wards:
        # Get all beds associated with this ward through rooms
    ...
    # 4. Recent Admissions (limit 5)
    recent_adms_raw = db.query(Admission).order_by(Admission.admission_date.desc()).limit(5).all()
    recent_admissions = []
    
    for adm in recent_adms_raw:
        # Load relationships manually if needed, or SQLAlchemy loads them.
        # Ensure we don't break if relationships are missing or lazy-loaded
